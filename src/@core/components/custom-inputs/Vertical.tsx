...
const CustomInputVertical = (props: CustomInputVerticalProps) => {
  ...
  // Vars
  const { title, value, content, asset, isDisabled } = data
    ...
    return (
      <Grid {...gridProps}>
        ...
        >
          ...
          {type === 'radio' ? (
            <RadioInput  disabled={isDisabled} name={name} color={color} value={value} onChange={handleChange} checked={selected === value} />
          ) : (
            <CheckboxInput
              color={color}
              name={`${name}-${value}`}
